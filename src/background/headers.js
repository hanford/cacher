...
function getCachingType (response) {
	var tab = chromeTabs.findById(response.tabId)

	if (tab !== undefined) {
		var savedHost = database.findHostsWithUrl(tab.url)
		if (savedHost !== null) {
...
function headerManipulator (responseHeaders) {
...
var onHeadersReceived = function(response) {
	...
	if (cachingType != CACHE_CONTROL.IGNORE && cachingType !== false) {
